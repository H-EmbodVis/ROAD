# Modified from Uni3D: reduced to the Uni3D-I evaluation path.
import logging
...
class Group(nn.Module):
    ...
    def forward(self, xyz, color):
        ...
        indices = indices + torch.arange(batch_size, device=xyz.device).view(-1, 1, 1) * num_points
        indices = indices.reshape(-1)
        neighborhoods = xyz.reshape(batch_size * num_points, 3)[indices]
        neighborhoods = neighborhoods.reshape(batch_size, self.num_group, self.group_size, 3)
...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        ...
        features = torch.cat([global_features.expand(-1, -1, point_count), features], dim=1)
...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, args):
        ...
        self.patch_dropout = (
            PatchDropout(args.patch_dropout) if args.patch_dropout > 0 else nn.Identity()
        )
