# Modified from Step1X-3D: removed private debugging dependencies.
import os
...
class BaseSystem(pl.LightningModule, Updateable, SaverMixin):
    ...
    def on_validation_batch_end(self, outputs, batch, batch_idx):
        ...
        for i in len(outputs):
            for j in outputs[i]['meshes']:
                if (
                        j.verts is not None
                        and j.verts.shape[0] > 0
                        and j.faces is not None
                        and j.faces.shape[0] > 0
                ):
                    self.save_mesh(
