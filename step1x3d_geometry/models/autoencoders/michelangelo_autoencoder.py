# Modified from Step1X-3D: removed experiment-only visualization and private paths.
import os
...
VALID_EMBED_TYPES = ["identity", "", "learned_fourier", "siren"]
...
class PerceiverCrossAttentionEncoder(ModelMixin, nn.Module):
    ...
    def _forward(self, pc, feats, sharp_pc=None, sharp_feat=None):
        ...
        if self.use_downsample:
            ...
            sampled_coords_output = pc.view(bs * N, -1)[idx].view(bs, -1, 3)
            ...
            if self.with_sharp_data:
                ...
                sharp_coords = sharp_pc.view(bs * N, -1)[idx].view(bs, -1, 3)
                sampled_coords_output = torch.cat([sampled_coords_output, sharp_coords], dim=1)
        ...
        return latents, sampled_coords_output
...
class MichelangeloAutoencoder(BaseModule):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.pretrained_model_name_or_path != "":
            local_model_path = os.path.join(self.cfg.pretrained_model_name_or_path, self.cfg.subfolder)
    ...
    def encode(
        ...
        shape_embeds ,sampled_coord = self.encoder(
            pc, feats, sharp_pc, sharp_feats
        )  # B, num_latents, width
        ...
        return shape_embeds, kl_embed, posterior, sampled_coord
