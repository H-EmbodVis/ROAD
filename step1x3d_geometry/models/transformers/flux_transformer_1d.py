# Modified from Step1X-3D for the focused baseline/Uni3D-REPA release.
...
class FluxTransformer1DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        #rapa_SRA
        s_encoder_depth:int = 6,
        t_encoder_depth: int =2,
        projector_dim=2048,
        is_cls_token: bool = False,
        z_dims:int = 1024,
        ...
        self.s_encoder_depth = s_encoder_depth
        self.t_encoder_depth = t_encoder_depth
        self.z_dims = z_dims
        ...
        if self.is_cls_token:
            self.cls_in = nn.Linear(self.z_dims,self.inner_dim,bias=True)
            self.cls_out = nn.Linear(self.inner_dim, self.z_dims, bias=True)
            self.cls_norm = nn.LayerNorm(self.inner_dim, elementwise_affine=True, eps=1e-6)
            self.norm_out_cls = AdaLayerNormContinuous(
                self.inner_dim, self.inner_dim, elementwise_affine=False, eps=1e-6
            )
        ...
        #self.projectors = nn.ModuleList([
            #build_mlp(width, projector_dim, z_dim) for z_dim in z_dims
        #])
    ...
    def forward(
        ...
        if self.is_cls_token:
            cls_states=self.cls_in(cls_input)
            cls_states=self.cls_norm(cls_states)
        ...
        for layer, block in enumerate(self.transformer_blocks):
            ...
            if ((layer + 1) == self.s_encoder_depth) and self.s_encoder_depth <= self.config.num_layers:
                zs = hidden_states.clone()

            if (layer + 1) == self.t_encoder_depth:
                zs_t = hidden_states.clone().detach()


            #vis(encoder_hidden_states)
        ...
        for layer, block in enumerate(self.single_transformer_blocks):
            ...
            if ((layer + 1) == (self.s_encoder_depth-self.config.num_layers)) and self.s_encoder_depth > self.config.num_layers:
                zs = hidden_states.clone()


            #vis(hidden_states[:,:encoder_hidden_states.shape[1],...])

        if self.is_cls_token:
            hidden_states = hidden_states[:, encoder_hidden_states.shape[1]:, ...]

            cls_out = hidden_states[:, 0, :].clone().unsqueeze(1)  # [2,1,1280]

            hidden_states = hidden_states[:, 1:, ...]

            # final layer
            hidden_states = self.norm_out(hidden_states, temb)
            # cls = self.cls_out(hidden_states[:, :1, :])
            hidden_states = self.proj_out(hidden_states)

            cls_out = self.norm_out_cls(cls_out, temb)
            cls_out = self.cls_out(cls_out)  # [2,2048,1280]
            ...
            if not return_dict:
                return (hidden_states,), zs, cls_out

            return Transformer1DModelOutput(sample=hidden_states), zs , zs_t, cls_out
            ...
            return Transformer1DModelOutput(sample=hidden_states) ,zs, zs_t,None
...
class FluxDenoiser(BaseModule):
    ...
    def configure(self) -> None:
        ...
        self.dit_model = FluxTransformer1DModel(
            ...
            is_cls_token= self.cfg.is_cls_token,
            s_encoder_depth=self.cfg.s_encoder_depth,
            t_encoder_depth=self.cfg.t_encoder_depth,
            z_dims= self.cfg.z_dims,
        )
        if (
        ...
        if self.cfg.pretrained_model_name_or_path:
            ...
            #ckpt = extract_denoiser_model(ckpt)
    ...
    def forward(
        ...
        return_dict: bool = True,
        cls_input: Optional[torch.FloatTensor] = None,
    ):
        r"""
        Args:
        ...
        output,zs,zs_t,cls_token = self.dit_model(
            model_input,
            timestep,
        ...
        return output,zs,zs_t,cls_token
